    ...
    private void showLichSuThi() {
        firebaseFirestore.collection("LichSuThi")
                ...
                .addOnCompleteListener(task -> {
                    //để xác định hành động sau khi truy vấn hoàn thành.
                    if (task.isSuccessful()) {
                        List<Map<String, Object>> lichSuThiList = new ArrayList<>();
                        for (QueryDocumentSnapshot document : task.getResult()) {
                            //dùng để lặp qua tất cả các tài liệu (QueryDocumentSnapshot) trong QuerySnapshot
                            //QueryDocumentSnapshot document: Mỗi lần lặp, biến document sẽ chứa một tài liệu trong QuerySnapshot.
